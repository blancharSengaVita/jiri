    ...
    <ul>
        @foreach($this->filteredContacts as $contact)
            <li wire:key="{{$contact->id}}">
                ...
                <input wire:click="addContactToContactsToAddToJiri({{ $contact->id }})" value="{{ $contact->name }}" type="checkbox" id="contact-{{ $contact->id }}">
            </li>
        @endforeach
    </ul>
    ...
    <ul dusk="@selectedList">
        @foreach($contactsToAddToJiri as $contact)
            <li>
                ...
                <input
{{--                    Faire l'inverse d'ajouter un contacter--}}
{{--                    wire:click="addContactToContactsToAddToJiri({{ $contact->id }})" --}}
                    value="{{ $contact->name }}"
